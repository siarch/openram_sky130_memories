...
class replica_column(bitcell_base_array):
    ...
    def __init__(self, name, rows, rbl, replica_bit, column_offset=0):
        ...
        #region debug-point replica-column-constraint
        if layer_props.replica_column.even_rows:
            # Log the effective geometry checks before enforcing sky130 mirroring rules.
            debug.info(
                1,
                "replica_column constraint check rows={} left_rbl={} right_rbl={} replica_bit={} netlist_only={} total_size={}".format(
                    rows,
                    self.left_rbl,
                    self.right_rbl,
                    replica_bit,
            ...
            # The mirroring rule is only needed when generating physical views.
    ...
    def create_layout(self):
        ...
        #region debug-point replica-column-layout-size
        debug.info(
            1,
            "replica_column layout size width={} height={} total_size={} column_offset={}".format(
                self.width,
                self.height,
                self.total_size,
                self.column_offset,
