...
# #region debug-point R:sram-1bank-report-event
...
class sram_1bank(design, verilog, lef):
    ...
    def create_netlist(self):
        ...
        # #region debug-point S:before-add-modules
        _dbg_report("S", "compiler/modules/sram_1bank.py:before_add_modules", "[DEBUG] before add_modules()", {"name": self.name})
        # #endregion
        self.add_modules()
        # #region debug-point T:after-add-modules
        _dbg_report("T", "compiler/modules/sram_1bank.py:after_add_modules", "[DEBUG] after add_modules()", {"inst_count": len(self.insts), "mod_count": len(self.mods)})
        # #endregion
        # #region debug-point U:before-add-pins
        _dbg_report("U", "compiler/modules/sram_1bank.py:before_add_pins", "[DEBUG] before add_pins()", {"pin_count": len(self.pins)})
        # #endregion
        self.add_pins()
        # #region debug-point V:after-add-pins
        _dbg_report("V", "compiler/modules/sram_1bank.py:after_add_pins", "[DEBUG] after add_pins()", {"pin_count": len(self.pins)})
        # #endregion
        # #region debug-point W:before-create-modules
        _dbg_report("W", "compiler/modules/sram_1bank.py:before_create_modules", "[DEBUG] before create_modules()", {"inst_count": len(self.insts)})
        # #endregion
        self.create_modules()
        # #region debug-point X:after-create-modules
        _dbg_report("X", "compiler/modules/sram_1bank.py:after_create_modules", "[DEBUG] after create_modules()", {"inst_count": len(self.insts), "mod_count": len(self.mods)})
        # #endregion
    ...
    def add_modules(self):
        # #region debug-point Y:before-bitcell-create
        _dbg_report("Y", "compiler/modules/sram_1bank.py:before_bitcell_create", "[DEBUG] before bitcell create", {"bitcell": OPTS.bitcell})
        # #endregion
        self.bitcell = factory.create(module_type=OPTS.bitcell)
        # #region debug-point Z:after-bitcell-create
        _dbg_report("Z", "compiler/modules/sram_1bank.py:after_bitcell_create", "[DEBUG] after bitcell create", {"bitcell_mod": getattr(self.bitcell, "name", None)})
        # #endregion
        self.dff = factory.create(module_type="dff")

        # Create the bank module (up to four are instantiated)
        # #region debug-point AA:before-bank-create
        _dbg_report("AA", "compiler/modules/sram_1bank.py:before_bank_create", "[DEBUG] before bank create", {"module_name": "bank"})
        # #endregion
        self.bank = factory.create("bank", sram_config=self.sram_config, module_name="bank")
        # #region debug-point AB:after-bank-create
        _dbg_report("AB", "compiler/modules/sram_1bank.py:after_bank_create", "[DEBUG] after bank create", {"bank_mod": getattr(self.bank, "name", None), "num_spare_cols": getattr(self.bank, "num_spare_cols", None)})
        # #endregion

        self.num_spare_cols = self.bank.num_spare_cols

        # Create the address and control flops (but not the clk)
        # #region debug-point AC:before-row-dff-create
        _dbg_report("AC", "compiler/modules/sram_1bank.py:before_row_dff_create", "[DEBUG] before row dff create", {"rows": self.row_addr_size})
        # #endregion
        self.row_addr_dff = factory.create("dff_array", module_name="row_addr_dff", rows=self.row_addr_size, columns=1)
        # #region debug-point AD:after-row-dff-create
        _dbg_report("AD", "compiler/modules/sram_1bank.py:after_row_dff_create", "[DEBUG] after row dff create", {"row_dff_mod": getattr(self.row_addr_dff, "name", None)})
        # #endregion
        ...
        # #region debug-point AE:before-control-logic-import
        _dbg_report("AE", "compiler/modules/sram_1bank.py:before_control_logic_import", "[DEBUG] before control logic import", {"control_logic": OPTS.control_logic})
        # #endregion
        c = reload(import_module("." + OPTS.control_logic, "openram.modules"))
        self.mod_control_logic = getattr(c, OPTS.control_logic)
        # #region debug-point AF:after-control-logic-import
        _dbg_report("AF", "compiler/modules/sram_1bank.py:after_control_logic_import", "[DEBUG] after control logic import", {"control_logic": OPTS.control_logic})
        # #endregion

        # Create the control logic module for each port type
        if len(self.readwrite_ports) > 0:
            # #region debug-point AG:before-control-logic-rw-create
            _dbg_report("AG", "compiler/modules/sram_1bank.py:before_control_logic_rw_create", "[DEBUG] before control_logic_rw create", {"num_rows": self.num_rows, "word_size": self.word_size})
            # #endregion
            self.control_logic_rw = self.mod_control_logic(num_rows=self.num_rows,
                                                           ...
                                                           sram=self,
                                                           port_type="rw")
            # #region debug-point AH:after-control-logic-rw-create
            _dbg_report("AH", "compiler/modules/sram_1bank.py:after_control_logic_rw_create", "[DEBUG] after control_logic_rw create", {"control_logic_rw": getattr(self.control_logic_rw, "name", None)})
            # #endregion
