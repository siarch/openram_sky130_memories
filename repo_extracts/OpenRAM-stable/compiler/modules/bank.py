...
# #region debug-point CA:bank-report-event
...
class bank(design):
    ...
    def __init__(self, sram_config, name=""):
        ...
        # #region debug-point CB:bank-ctor-enter
        _dbg_report("CB", "compiler/modules/bank.py:ctor_enter", "[DEBUG] bank ctor enter", {"name_arg": name})
        # #endregion
        ...
        super().__init__(name)
        # #region debug-point CC:bank-after-super
        _dbg_report("CC", "compiler/modules/bank.py:after_super", "[DEBUG] bank after super()", {"name": self.name, "word_size": self.word_size, "num_words": self.num_words})
        # #endregion
        ...
        # #region debug-point CD:bank-before-create-netlist
        _dbg_report("CD", "compiler/modules/bank.py:before_create_netlist", "[DEBUG] bank before create_netlist()", {"name": self.name})
        # #endregion
        self.create_netlist()
        # #region debug-point CE:bank-after-create-netlist
        _dbg_report("CE", "compiler/modules/bank.py:after_create_netlist", "[DEBUG] bank after create_netlist()", {"name": self.name})
        # #endregion
    ...
    def create_netlist(self):
        ...
        # #region debug-point CF:bank-before-compute-sizes
        _dbg_report("CF", "compiler/modules/bank.py:before_compute_sizes", "[DEBUG] bank before compute_sizes()", {"name": self.name})
        # #endregion
        self.compute_sizes()
        # #region debug-point CG:bank-after-compute-sizes
        _dbg_report("CG", "compiler/modules/bank.py:after_compute_sizes", "[DEBUG] bank after compute_sizes()", {"name": self.name, "bank_addr_size": getattr(self, "bank_addr_size", None)})
        # #endregion
        # #region debug-point CH:bank-before-add-modules
        _dbg_report("CH", "compiler/modules/bank.py:before_add_modules", "[DEBUG] bank before add_modules()", {"name": self.name})
        # #endregion
        self.add_modules()
        # #region debug-point CI:bank-after-add-modules
        _dbg_report("CI", "compiler/modules/bank.py:after_add_modules", "[DEBUG] bank after add_modules()", {"name": self.name})
        # #endregion
        # #region debug-point CJ:bank-before-add-pins
        _dbg_report("CJ", "compiler/modules/bank.py:before_add_pins", "[DEBUG] bank before add_pins()", {"name": self.name})
        # #endregion
        self.add_pins() # Must create the replica bitcell array first
        # #region debug-point CK:bank-after-add-pins
        _dbg_report("CK", "compiler/modules/bank.py:after_add_pins", "[DEBUG] bank after add_pins()", {"name": self.name, "pin_count": len(self.pins)})
        # #endregion
        # #region debug-point CL:bank-before-create-instances
        _dbg_report("CL", "compiler/modules/bank.py:before_create_instances", "[DEBUG] bank before create_instances()", {"name": self.name})
        # #endregion
        self.create_instances()
        # #region debug-point CM:bank-after-create-instances
        _dbg_report("CM", "compiler/modules/bank.py:after_create_instances", "[DEBUG] bank after create_instances()", {"name": self.name, "inst_count": len(self.insts)})
        # #endregion
    ...
    def add_modules(self):
        ...
        if local_array_size > 0:
            ...
            # #region debug-point CN:bank-before-bitcell-array
            _dbg_report("CN", "compiler/modules/bank.py:before_bitcell_array", "[DEBUG] bank before bitcell_array create", {"local_array_size": local_array_size})
            # #endregion
            self.bitcell_array = factory.create(module_type="global_bitcell_array",
                                                ...
                                                left_rbl=left_rbl,
                                                right_rbl=right_rbl)
            # #region debug-point CO:bank-after-bitcell-array
            _dbg_report("CO", "compiler/modules/bank.py:after_bitcell_array", "[DEBUG] bank after bitcell_array create", {"bitcell_array": getattr(self.bitcell_array, "name", None)})
            # #endregion
        else:
            # #region debug-point CN:bank-before-bitcell-array
            _dbg_report("CN", "compiler/modules/bank.py:before_bitcell_array", "[DEBUG] bank before bitcell_array create", {"local_array_size": local_array_size})
        ...
        self.port_address = []
        for port in self.all_ports:
            # #region debug-point CP:bank-before-port-address
            _dbg_report("CP", "compiler/modules/bank.py:before_port_address", "[DEBUG] bank before port_address create", {"port": port})
            # #endregion
            ...
            # #region debug-point CQ:bank-after-port-address
            _dbg_report("CQ", "compiler/modules/bank.py:after_port_address", "[DEBUG] bank after port_address create", {"port": port, "port_address": getattr(self.port_address[-1], "name", None)})
            # #endregion

        self.port_data = []
        self.bit_offsets = self.get_column_offsets()
        for port in self.all_ports:
            # #region debug-point CR:bank-before-port-data
            _dbg_report("CR", "compiler/modules/bank.py:before_port_data", "[DEBUG] bank before port_data create", {"port": port, "bit_offsets_len": len(self.bit_offsets) if self.bit_offsets is not None else None})
            # #endregion
            ...
            # #region debug-point CS:bank-after-port-data
            _dbg_report("CS", "compiler/modules/bank.py:after_port_data", "[DEBUG] bank after port_data create", {"port": port, "port_data": getattr(self.port_data[-1], "name", None)})
            # #endregion
