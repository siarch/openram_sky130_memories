...
# #region debug-point BA:factory-report-event
...
class sram_factory:
    ...
    def create(self, module_type, module_name=None, **kwargs):
        ...
        dbg_types = {
            "bank",
            "sky130_capped_replica_bitcell_array",
            "capped_replica_bitcell_array",
            "sky130_replica_bitcell_array",
            "replica_bitcell_array",
            "sky130_bitcell_array",
            "bitcell_array",
        }
        # #region debug-point BB:factory-before-ctor
        if real_module_type in dbg_types:
            _dbg_report(
                "BB",
                "compiler/sram_factory.py:before_ctor",
                "[DEBUG] factory before ctor",
                {"real_module_type": real_module_type, "module_name": module_name, "kwargs_keys": sorted(list(kwargs.keys()))},
            )
        # #endregion
        try:
            ...
            if real_module_type in dbg_types:
                _dbg_report(
                    "BC",
                    "compiler/sram_factory.py:ctor_exception",
                    "[DEBUG] factory ctor exception",
                    {"real_module_type": real_module_type, "module_name": module_name, "exc_type": type(e).__name__, "exc": repr(e), "traceback": traceback.format_exc()[:4000]},
                )
            raise
        # #region debug-point BD:factory-after-ctor
        if real_module_type in dbg_types:
            _dbg_report(
                "BD",
                "compiler/sram_factory.py:after_ctor",
                "[DEBUG] factory after ctor",
                {"real_module_type": real_module_type, "module_name": module_name, "obj_name": getattr(obj, "name", None)},
            )
        # #endregion
