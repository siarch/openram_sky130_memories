...
# #region debug-point J:sram-report-event
...
class sram():
    ...
    def __init__(self, sram_config=None, name=None):
        ...
        # #region debug-point K:before-module-ctor
        _dbg_report("K", "compiler/sram.py:before_module_ctor", "[DEBUG] before sram_1bank()", {"name": name, "word_size": self.word_size, "num_words": self.num_words})
        # #endregion

        self.s = sram(name, sram_config)
        # #region debug-point L:after-module-ctor
        _dbg_report("L", "compiler/sram.py:after_module_ctor", "[DEBUG] after sram_1bank()", {"module_name": getattr(self.s, "name", None)})
        # #endregion

        # #region debug-point M:before-create-netlist
        _dbg_report("M", "compiler/sram.py:before_create_netlist", "[DEBUG] before create_netlist()", {"module_name": getattr(self.s, "name", None)})
        # #endregion
        self.s.create_netlist()
        # #region debug-point N:after-create-netlist
        _dbg_report("N", "compiler/sram.py:after_create_netlist", "[DEBUG] after create_netlist()", {"module_name": getattr(self.s, "name", None)})
        # #endregion
        if not OPTS.netlist_only:
            # #region debug-point O:before-create-layout
            _dbg_report("O", "compiler/sram.py:before_create_layout", "[DEBUG] before create_layout()", {"module_name": getattr(self.s, "name", None)})
            # #endregion
            self.s.create_layout()
            # #region debug-point P:after-create-layout
            _dbg_report("P", "compiler/sram.py:after_create_layout", "[DEBUG] after create_layout()", {"module_name": getattr(self.s, "name", None)})
            # #endregion
    ...
    def save(self):
        ...
        # #region debug-point Q:before-sp-write
        _dbg_report("Q", "compiler/sram.py:before_sp_write", "[DEBUG] before sp_write()", {"spname": spname})
        # #endregion
