...
# #region debug-point FA:replica-array-report-event
...
class replica_bitcell_array(bitcell_base_array):
    ...
    def __init__(self, rows, cols, rbl=None, left_rbl=None, right_rbl=None, name=""):
        # #region debug-point FB:ctor-enter
        _dbg_report("FB", "technology/sky130/custom/replica_bitcell_array.py:ctor_enter", "[DEBUG] replica_bitcell_array ctor enter", {"rows": rows, "cols": cols})
        # #endregion
        ...
        # #region debug-point FC:before-create-netlist
        _dbg_report("FC", "technology/sky130/custom/replica_bitcell_array.py:before_create_netlist", "[DEBUG] before create_netlist()", {"name": self.name})
        # #endregion
        self.create_netlist()
        # #region debug-point FD:after-create-netlist
        _dbg_report("FD", "technology/sky130/custom/replica_bitcell_array.py:after_create_netlist", "[DEBUG] after create_netlist()", {"name": self.name})
        # #endregion
    ...
    def add_modules(self):
        ...
        """
        # Bitcell array
        # #region debug-point FE:before-bitcell-array
        _dbg_report("FE", "technology/sky130/custom/replica_bitcell_array.py:before_bitcell_array", "[DEBUG] before bitcell_array create", {"cols": self.column_size, "rows": self.row_size})
        # #endregion
        self.bitcell_array = factory.create(module_type="bitcell_array",
                                            column_offset=1 + len(self.left_rbl),
                                            cols=self.column_size,
                                            rows=self.row_size)
        # #region debug-point FF:after-bitcell-array
        _dbg_report("FF", "technology/sky130/custom/replica_bitcell_array.py:after_bitcell_array", "[DEBUG] after bitcell_array create", {"bitcell_array": getattr(self.bitcell_array, "name", None)})
        # #endregion
