...
summary="/tmp/batch_timeout_1r1w_summary.tsv"
: > "$summary"
...
while IFS= read -r script_path; do
    ...
    echo "RUN $mem_id"
    if timeout 300 bash "$script_path" >"/tmp/${mem_id}.timeout.log" 2>&1; then
...
python3 /Sky130_memory/scripts/postprocess_and_report.py >/tmp/postprocess_timeout_1r1w.log 2>&1 || true
