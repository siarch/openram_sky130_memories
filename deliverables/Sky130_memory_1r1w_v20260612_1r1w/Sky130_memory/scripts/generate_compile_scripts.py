...
def get_port_cfg(ports: str):
    ports = (ports or "1rw").strip().lower()
    if ports == "1rw":
        return {
            "num_rw_ports": 1,
            "num_r_ports": 0,
    ...
    raise SystemExit(f"Unsupported ports setting: {ports}")
...
def main():
    ...
    for m in memories:
        mem_id = m["id"]
        ports = m.get("ports", "1rw")
        word_size = int(m.get("word_size", "0") or "0")
        words = int(m.get("words", "0") or "0")
        write_size = int(m.get("write_size", str(word_size)) or str(word_size))
        human_byte_size = format_human_byte_size(word_size, words)
        port_cfg = get_port_cfg(ports)
        ...
        cfg_text = "\n".join(
            [
                ...
                f"num_words = {words}",
                f"human_byte_size = \"{human_byte_size}\"",
                f"write_size = {write_size}",
                f"num_rw_ports = {port_cfg['num_rw_ports']}",
                f"num_r_ports = {port_cfg['num_r_ports']}",
                f"num_w_ports = {port_cfg['num_w_ports']}",
                "num_spare_rows = 1",
                "num_spare_cols = 1",
                f"ports_human = '{port_cfg['ports_human']}'",
                "import os",
